...
from .temporal.temporal_blocks import (
    TemporalAvgPool1d,
    TemporalMaxPool1d,
    TemporalConv1d,
    TemporalGlobalAvgPool,
)
...
def create_block(t_block: TBlock) -> nn.Module:
    if t_block.name == "Block1":
        return Block1(
            in_channels=t_block.in_channels,
...
class Block1(nn.Module):
    def __init__(
        ...
        if temporal_pool_type == "max":
            self.pool1d = TemporalMaxPool1d(
                temporal_ksize,
                stride=temporal_stride,
                padding=(temporal_ksize - 1) // 2,
            )
        elif temporal_pool_type == "avg":
            self.pool1d = TemporalAvgPool1d(
                temporal_ksize,
                stride=temporal_stride,
                padding=(temporal_ksize - 1) // 2,
            )
...
class Block2(nn.Module):
    def __init__(self, in_channels, out_channels, temporal_ksize=3, temporal_stride=2):
...
class Block3(nn.Module):
    def __init__(self, in_channels, spatial_ksize=3, temporal_ksize=3):
...
class Block4(nn.Module):
    def __init__(
        self, in_channels, out_channels, temporal_ksize=3, temporal_stride=2, cg_ksize=3
    ):
