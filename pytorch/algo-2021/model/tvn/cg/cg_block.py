...
class ContextGatedConv2d(nn.Conv2d):
    def __init__(
        ...
        super(ContextGatedConv2d, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
        ...
        if kernel_size == 1:
            ...
            if in_channels // 16:
                ...
                self.g = in_channels
            self.ci = nn.Linear(
                self.g, out_channels // (in_channels // self.g), bias=False
            )
    ...
    def forward(self, x):
        if self.ind:
            ...
            if self.g > 3:
                ...
                oc = (
                    self.ci(self.act(self.ci_bn2(ce2).transpose(2, 1)))
                    .transpose(2, 1)
                    .contiguous()
                )
            ...
            oc = self.gd2(oc)
            out = self.sig(
                out.view(b, 1, c, self.ks, self.ks)
                + oc.view(b, self.oc, 1, self.ks, self.ks)
            )
            x_un = self.unfold(x)
            b, _, l = x_un.size()
            out = (out * weight.unsqueeze(0)).view(b, self.oc, -1)
            return torch.matmul(out, x_un).view(
                b, self.oc, int(np.sqrt(l)), int(np.sqrt(l))
            )
