...
class CoAttnWithAttnPoolingWithMCGClassifier(VideoClassifier):
    def __init__(self, config):
        ...
        for modal in modal_list:
            for other_modal in modal_list:
                ...
                modal_cg_dict[f"{modal}_{other_modal}"] = StackedModalContextGating(
                    dim_a=self.config["pooling_dim"],
                    dim_b=self.config["pooling_dim"],
                    layer=1,
                )

        self.co_attention_dict = nn.ModuleDict(co_attention_dict)
        self.modal_cg_dict = nn.ModuleDict(modal_cg_dict)
    ...
    def forward(self, inputs):
        ...
        need_to_cat_feature = list()
        for item in self.modal_cg_dict:
            modal_name = item.split("_")
            modal_cg = self.modal_cg_dict[item]
            temp_result = modal_cg(
                {
                    "feature_a": feature_dict[modal_name[0]],
                    "feature_b": feature_dict[modal_name[1]],
                }
            )
            need_to_cat_feature.append(temp_result["feature_a"])

        # feature = torch.cat(list(feature_dict.values()), dim=-1)
        feature = torch.cat(need_to_cat_feature, dim=-1)
